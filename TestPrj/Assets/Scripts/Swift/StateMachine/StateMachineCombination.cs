...
namespace Swift
{
    ...
    public class StateMachineCombination : StateMachine
    {
        ...
        {
            base.Start();
            foreach (var sm in sms) sm.Start();
        }

        // 销毁状态机，不能再次启动
        public override void Destroy()
        {
            foreach (var sm in sms) sm.Destroy();
            base.Destroy();
        }

        public override void Pause()
        {
            foreach (var sm in sms) sm.Pause();
            base.Pause();
        }

        public override void Resume()
        {
            foreach (var sm in sms) sm.Resume();
            base.Resume();
        }

        public override void Run(Fix64 te)
        {
            foreach (var sm in sms) sm.Run(te);
            base.Run(te);
        }
        ...
        public override void LogAllStates(Action<string> logger)
        {
            ...
            foreach (var sm in sms) sm.LogAllStates(logger);
        }
